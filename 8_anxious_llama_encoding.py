...
Uses MNI-space ROI voxel data directly (no SRM). For each LLM layer (12, 16, 20),
fits a stimulus->response encoding model on the training subjects' voxels and
predicts the held-out subject's voxels. Nested LOSO selects the ridge penalty.
Outputs one mean-R per held-out subject (averaged across ROI voxels) so groups
can be compared later.

Because subjects are anatomically aligned (MNI), voxel v in the held-out subject
corresponds to voxel v in the training subjects, so the model fit on training
voxels applies directly.
'''
...
BASE_OUT_DIR = "../outputs/encoding_results_anxious_conditions"
...
def design_dir_for(prompt_version, condition, movie):
    """
    Matches the output layout of 3_lanczos_and_FIR_anxious.py.
    """
    return f"../outputs/{prompt_version}_condition_{condition}_{movie}_trimmed"

def load_responses_all_movies(movies, roi, prompt_version, condition):
    """
    Load ROI responses for every movie (layer-independent), collapsed to a single
    ROI-mean time series per subject/movie via nanmean (ignores any NaN voxels
    rather than propagating them across the whole mean).

    n_trs per movie comes from the condition-specific design matrix, since that's
    the canonical N_TR the FIR-builder used for that movie (from trim_qc_log.csv)
    -- it does not depend on condition, but read it to reuse load_design's file
    layour directly and guarantee TR-count agreement with X_by_movie built later
    in load_designs_all_movies.
...
def load_designs_all_movies(movies, layer, prompt_version, condition):
    """
    X_by_movie: {movie: (n_trs_movie, n_featres)} for one layer, for the given
    condition.
    """
    return {
        movie: load_design(design_dir_for(prompt_version, condition, movie), layer)
        for movie in movies
    }
...
def nested_loso_lomo(X_by_movie, responses_by_movie, movies, subjects):
    '''
    Between-subject, between-movie encoding with nested LOSO x LOMO, and one 
    lambda per (held-out subject, held-out movie) outer fold.

    X_by_movie: {movie: (n_trs_movie, n_features)} -- each movie has its own design
        matrix, shared across subjects for that movie.
    responses_by_movie: {movie: {sid: (n_trs_movie, n_voxel_or_1)}} in MNI space
        (voxels aligned across subjects, or ROI-mean).

    Fits X -> stacked training-subject/training-movie responses, predicts held-out 
    subject's responses, correlates per output column, averages across columns.

    Leave out both one subject and one movie, cycle through all movies and 
    concatenate predictions across movies for the final correlation.

    Returns:
        subject_mean_R: {sid: mean_R across columns, pooled over all movies}.
        subject_predict: {sid: {movie: (n_trs_movie, 1) predicted array}} --
            per-TR predictions, kept for downstream chunk-level analysis.
    '''
    subject_predict = {
    ...
    for out_sid in subjects:
        ...
        for out_movie in movies:
            ...
            for val_movie in train_movies:
                ...
                # X for the inner loop depends only on how many subjects contribute
                # (len(train_sids)-1), not on which ones, so it's identical across
                # val_sid and only needs building once per (out_sid, out_movie, val_movie).
                ...
                for val_sid in train_sids:
                    ...
                    Y_val = responses_by_movie[val_movie][val_sid] # held out subject, held out movie
...
def main():
    ...
    ap.add_argument("--movies", required=True,
                    help="comma-separated movie names, e.g. chatter,sintel")
    ...
    ap.add_argument("--run-tag", default=None,
                    help="Optional label for this run; results go in "
                        "BASE_OUT_DIR/{prompt_version}_condition_{condition}/<run-tag>/. "
                        "Defaults to no extra subfolder.")
    args = ap.parse_args()

    OUT_DIR = os.path.join(BASE_OUT_DIR, f"{args.prompt_version}_condition_{args.condition}") # (2) per-run folder
    if args.run_tag:
        OUT_DIR = os.path.join(OUT_DIR, args.run_tag)
    os.makedirs(OUT_DIR, exist_ok= True)

    movies = args.movies.split(",")

    run_config = {                                     # (3) provenance + timing
    ...
    print("TRAINING ENCODING WITH NOT SRM-TRANSFORMED DATA, ZSCORED")
    print(f"=== Encoding: movies={movies}, roi={args.roi}, "
        f"condition={args.condition}, prompt_version={args.prompt_version} "
        f"(MNI, no SRM, LOSO x LOMO, ROI-mean) ===")

    print("[1/4] Loading ROI responses ...")
    responses_by_movie, n_trs_by_movie, subjects = load_responses_all_movies(
        movies, args.roi, args.prompt_version, args.condition
    )
    ...
    for layer in LAYERS:
        print(f"  ---Layer: {layer}---")
        X_by_movie = load_designs_all_movies(movies, layer, args.prompt_version, args.condition)
        for m in movies:
            assert X_by_movie[m].shape[0] == n_trs_by_movie[m], \
            f"design/response TR mismatch for movie {m}, layer {layer}"
        print("[2/4] Running nested LOSO x LOMO... ")
